...
( function( window, angular, undefined ) {
  ...
  angular.module( 'satellizer', [] ).constant( 'satellizer.config', {
    ...
    providers: {
      google: {
        ...
        redirectUri: currentUrl() + '/oauth2callback',
        scope: [ 'profile', 'email' ],
        scopePrefix: 'openid',
        scopeDelimiter: ' ',
    ...
    this.$get = [ '$q', 'satellizer.shared', 'satellizer.local', 'satellizer.oauth',
  ...
  } ] ).factory( 'satellizer.shared', [ '$q', '$window', '$location', 'satellizer.config',
    function( $q, $window, $location, config ) {
      ...
      shared.setToken = function( response, redirect ) {
        ...
        if ( !token && response ) {
          token = config.tokenRoot && response.data[ config.tokenRoot ] ? response.data[ config.tokenRoot ][
            config.tokenName
        ...
        if ( !token ) {
          ...
          // throw new Error( 'Expecting a token named "' + tokenName + '" but instead got: ' + JSON.stringify(
          //   response.data ) );
        }
  ...
  ] ).factory( 'satellizer.oauth', [ '$q', '$http', 'satellizer.config', 'satellizer.shared', 'satellizer.Oauth1',
    'satellizer.Oauth2',
  ...
  ] ).factory( 'satellizer.local', [ '$q', '$http', '$location', 'satellizer.utils', 'satellizer.shared',
    'satellizer.config',
  ...
  ] ).factory( 'satellizer.Oauth2', [ '$q', '$http', '$window', 'satellizer.popup', 'satellizer.utils',
    'satellizer.config',
    ...
    return function() {
      ...
      oauth1.open = function( options, userData ) {
        angular.extend( defaults, options );
        return popup.open( defaults.url, defaults.popupOptions, defaults.redirectUri ).then( function(
          response ) {
  ...
  } ] ).factory( 'satellizer.popup', [ '$q', '$interval', '$window', '$location', 'satellizer.config',
    'satellizer.utils',
...
( function() {
  ...
  object.btoa || ( object.btoa = function( input ) {
    ...
    for ( var block, charCode, idx = 0, map = chars, output = ''; str.charAt( idx | 0 ) || ( map = '=', idx % 1 ); output +=
      map.charAt( 63 & block >> 8 - idx % 1 * 8 ) ) {
  ...
  object.atob || ( object.atob = function( input ) {
    ...
    for ( var bc = 0, bs, buffer, idx = 0, output = ''; buffer = str.charAt( idx++ ); ~buffer && ( bs = bc % 4 ?
        bs * 64 + buffer : buffer, bc++ % 4 ) ? output += String.fromCharCode( 255 & bs >> ( -2 * bc & 6 ) ) : 0 ) {
